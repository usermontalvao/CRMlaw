...
const CATEGORY_COLORS: Record<StandardPetitionCategory, string> = {
  requerimento_administrativo: 'bg-blue-100 text-blue-700 border-blue-200',
  peticao_inicial: 'bg-emerald-100 text-emerald-700 border-emerald-200',
  recurso: 'bg-amber-100 text-amber-700 border-amber-200',
  contestacao: 'bg-purple-100 text-purple-700 border-purple-200',
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
        <div className="bg-gradient-to-r from-slate-900 via-slate-900 to-indigo-900 px-5 py-5 sm:px-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex items-start gap-4">
              ...
              <div>
                <div className="text-xs font-semibold uppercase tracking-wide text-slate-200">Modelos</div>
                <h3 className="mt-1 text-xl font-semibold text-white sm:text-2xl">Modelos de Documentos</h3>
                <p className="mt-1 text-sm text-slate-200">Crie, organize e gere documentos (contratos, petições e requerimentos) com campos dinâmicos.</p>
              </div>
            </div>

            <div className="flex items-center gap-2">
              <div className="rounded-xl bg-white/10 border border-white/15 px-3 py-2">
                <div className="text-[10px] uppercase tracking-wide text-slate-200 font-semibold">Modelos</div>
                <div className="text-white text-sm font-semibold">{petitionCounts.total}</div>
              </div>
              <div className="rounded-xl bg-white/10 border border-white/15 px-3 py-2">
                <div className="text-[10px] uppercase tracking-wide text-slate-200 font-semibold">Ativos</div>
                <div className="text-white text-sm font-semibold">{petitionCounts.active}</div>
              </div>
              <div className="rounded-xl bg-white/10 border border-white/15 px-3 py-2">
                <div className="text-[10px] uppercase tracking-wide text-slate-200 font-semibold">DOCX</div>
                <div className="text-white text-sm font-semibold">{petitionCounts.withFile}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="px-4 py-4 sm:px-6 border-b border-slate-200 bg-slate-50">
          <div className="inline-flex rounded-xl bg-white border border-slate-200 p-1 shadow-sm">
            <button
              ...
              className={`inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold transition ${activeView === 'generate' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-700 hover:bg-slate-50'}`}
            >
            ...
            <button
              ...
              className={`inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold transition ${activeView === 'list' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-700 hover:bg-slate-50'}`}
            >
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button onClick={handleOpenCreateModal} className="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-indigo-700">
          ...
          {loading ? (
            ...
            <div className="rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 p-8 text-center">
              ...
              <p className="mt-3 text-sm font-medium text-slate-600">Nenhuma petição encontrada</p>
              {!searchTerm && !categoryFilter && (
                <button onClick={handleOpenCreateModal} className="mt-4 inline-flex items-center gap-2 text-sm font-medium text-indigo-600 hover:text-indigo-700">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {filteredPetitions.map((petition) => (
                <div key={petition.id} className="group rounded-xl border border-slate-200 bg-white p-4 transition hover:border-slate-300 hover:shadow-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {!petition.is_active && <span className="inline-flex items-center rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-500">Inativo</span>}
                      </div>
                      <h4 className="mt-2 text-sm font-semibold text-slate-900 truncate">{petition.name}</h4>
                      {petition.description && <p className="mt-1 text-xs text-slate-500 line-clamp-2">{petition.description}</p>}
                    </div>
                    {petition.file_path && (
                  ...
                  <div className="mt-4 flex items-center gap-2">
                    <button onClick={() => handleViewPetition(petition)} className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg border border-slate-200 px-3 py-2 text-xs font-medium text-slate-700 hover:bg-slate-50">
                      <Eye className="h-3.5 w-3.5" />Ver
                    </button>
                    <button onClick={() => handleOpenFieldsModal(petition)} className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg border border-slate-200 px-3 py-2 text-xs font-medium text-slate-700 hover:bg-slate-50">
                      <Settings className="h-3.5 w-3.5" />Campos
                    </button>
                    <button onClick={() => handleOpenEditModal(petition)} className="inline-flex items-center justify-center rounded-lg border border-slate-200 p-2 text-slate-700 hover:bg-slate-50">
                      <Pencil className="h-3.5 w-3.5" />
                    </button>
                    <button onClick={() => handleDeletePetition(petition)} className="inline-flex items-center justify-center rounded-lg border border-red-200 p-2 text-red-600 hover:bg-red-50">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
          <div className="bg-gradient-to-r from-indigo-600 to-indigo-700 px-6 py-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-white">Gerar Documento</h3>
                <p className="text-sm text-indigo-100">Selecione a categoria, modelo e cliente</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-6">
            <div className="flex items-center justify-center gap-2 mb-6">
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium transition ${selectedCategory ? 'bg-indigo-100 text-indigo-700' : 'bg-indigo-600 text-white'}`}>
                <span className="w-5 h-5 rounded-full bg-current/20 flex items-center justify-center text-[10px] font-bold">1</span>
                Categoria
              </div>
              <div className="w-8 h-0.5 bg-slate-200" />
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium transition ${selectedCategory && selectedPetitionId ? 'bg-indigo-100 text-indigo-700' : selectedCategory ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-400'}`}>
                <span className="w-5 h-5 rounded-full bg-current/20 flex items-center justify-center text-[10px] font-bold">2</span>
                Modelo
              </div>
              <div className="w-8 h-0.5 bg-slate-200" />
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium transition ${selectedCategory && selectedPetitionId && selectedClientId ? 'bg-indigo-100 text-indigo-700' : selectedCategory && selectedPetitionId ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-400'}`}>
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                  {Object.entries(CATEGORY_LABELS).map(([key, label]) => {
                    ...
                    return (
                      ...
                      >
                        <p className={`text-sm font-medium truncate ${isSelected ? 'text-indigo-900' : 'text-slate-700'}`}>{label}</p>
                        <p className={`text-xs mt-0.5 ${isSelected ? 'text-indigo-600' : 'text-slate-400'}`}>
                          ...
                          >
                            <div className="flex items-start gap-3">
                              <div className={`flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center ${isSelected ? 'bg-indigo-100' : 'bg-slate-100'}`}>
                                <FileText className={`h-5 w-5 ${isSelected ? 'text-indigo-600' : 'text-slate-500'}`} />
                              </div>
                              <div className="flex-1 min-w-0">
                                <p className={`text-sm font-medium ${isSelected ? 'text-indigo-900' : 'text-slate-900'}`}>{petition.name}</p>
                                {petition.description && (
                                  <p className="text-xs text-slate-500 mt-0.5 line-clamp-2">{petition.description}</p>
                                )}
                              </div>
                              {isSelected && (
                                <div className="flex-shrink-0">
              ...
              {selectedPetition && selectedClientId && petitionFields.length > 0 && (
                <div ref={stepCamposRef} className="animate-in fade-in slide-in-from-top-2 duration-300 rounded-xl border border-slate-200 bg-slate-50 p-4">
                  <div className="flex items-center gap-2"><Settings className="h-4 w-4 text-slate-500" /><p className="text-sm font-semibold text-slate-900">Campos Personalizados</p></div>
                  <p className="mt-1 text-xs text-slate-500">Preencha os campos específicos desta petição.</p>
              ...
              {selectedCategory && selectedPetitionId && selectedClientId && (
                <div ref={stepGerarRef} className="animate-in fade-in slide-in-from-top-2 duration-300 pt-2">
                  <button 
                    ...
                    disabled={generating} 
                    className="w-full rounded-xl bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 px-6 py-4 text-sm font-semibold text-white transition inline-flex items-center justify-center gap-2 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
                  >
      ...
      {isFormModalOpen && createPortal(
        <div className="pericia-light-modal fixed inset-0 z-[80] flex items-center justify-center px-3 sm:px-6 py-4">
          ...
          <div className="pericia-light-modal__panel relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
            ...
            <div className="px-5 sm:px-6 py-5 border-b border-slate-200 flex items-start justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <h2 className="text-xl font-semibold text-slate-900">{editingPetition ? 'Editar Petição' : 'Nova Petição'}</h2>
                </div>
              </div>
              <button type="button" onClick={handleCloseFormModal} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-xl"><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={handleSavePetition} className="flex-1 overflow-y-auto">
              <div className="p-5 sm:p-6 space-y-4">
                <div><label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">Nome *</label><input type="text" value={formData.name} onChange={(e) => setFormData((prev) => ({ ...prev, name: e.target.value }))} className="w-full rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm" placeholder="Ex: Requerimento Administrativo INSS" required /></div>
                <div><label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">Descrição</label><input type="text" value={formData.description} onChange={(e) => setFormData((prev) => ({ ...prev, description: e.target.value }))} className="w-full rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm" placeholder="Breve descrição da petição" /></div>
                <div><label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">Categoria *</label><select value={formData.category} onChange={(e) => setFormData((prev) => ({ ...prev, category: e.target.value as StandardPetitionCategory }))} className="w-full rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm">{Object.entries(CATEGORY_LABELS).map(([key, label]) => (<option key={key} value={key}>{label}</option>))}</select></div>
                <div>
                  <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">
                  ...
                  {editingPetition?.file_path && !formFile ? (
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p className="text-sm font-medium text-slate-700 truncate">{editingPetition.file_name}</p>
                            <p className="text-xs text-slate-500">Template atual</p>
                          </div>
                        </div>
                    ...
                    <div className="flex items-center gap-3">
                      <label className="flex-1 cursor-pointer">
                        <div className="flex items-center justify-center gap-2 rounded-lg border-2 border-dashed border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600 hover:border-slate-300 hover:bg-slate-100">
                      ...
                      {formFile && (
                        <button type="button" onClick={() => setFormFile(null)} className="p-2 text-slate-400 hover:text-red-600">
                          <X className="h-4 w-4" />
                        </button>
                      )}
                    </div>
              ...
              <div className="border-t border-slate-200 bg-slate-50 px-4 sm:px-6 py-3">
                <div className="flex flex-col sm:flex-row gap-3 sm:justify-end">
                  <button type="button" onClick={handleCloseFormModal} className="px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-semibold text-slate-700 hover:bg-slate-100" disabled={formSaving}>Cancelar</button>
                  <button type="submit" disabled={formSaving} className="px-4 py-2.5 rounded-xl text-sm font-semibold inline-flex items-center justify-center gap-2 bg-indigo-600 text-white hover:bg-indigo-700 disabled:bg-slate-300 disabled:cursor-not-allowed">{formSaving && <Loader2 className="w-4 h-4 animate-spin" />}{editingPetition ? 'Salvar Alterações' : 'Criar Petição'}</button>
                </div>
              </div>
            </form>
          </div>
      ...
      {isFieldsModalOpen && fieldsPetition && createPortal(
        <div className="pericia-light-modal fixed inset-0 z-[80] flex items-center justify-center px-3 sm:px-6 py-4">
          ...
          <div className="pericia-light-modal__panel relative w-full max-w-3xl bg-white rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
            ...
            <div className="px-5 sm:px-6 py-5 border-b border-slate-200 flex items-start justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <h2 className="text-xl font-semibold text-slate-900 truncate max-w-md">{fieldsPetition.name}</h2>
                </div>
              </div>
              <button type="button" onClick={handleCloseFieldsModal} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-xl"><X className="w-5 h-5" /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-6">
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-3">
                  <h4 className="text-sm font-semibold text-slate-900">{editingField ? 'Editar Campo' : 'Adicionar Campo'}</h4>
                  {editingField && (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div><label className="mb-1 block text-xs font-medium text-slate-500">Nome do Campo</label><input type="text" value={newField.name} onChange={(e) => setNewField((prev) => ({ ...prev, name: e.target.value }))} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm" placeholder="Ex: Valor do Benefício" /></div>
                  <div><label className="mb-1 block text-xs font-medium text-slate-500">Placeholder</label><input type="text" value={newField.placeholder} onChange={(e) => setNewField((prev) => ({ ...prev, placeholder: e.target.value }))} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm" placeholder="Ex: VALOR_BENEFICIO" /></div>
                  <div><label className="mb-1 block text-xs font-medium text-slate-500">Tipo</label><select value={newField.field_type} onChange={(e) => setNewField((prev) => ({ ...prev, field_type: e.target.value as StandardPetitionFieldType }))} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm">{Object.entries(FIELD_TYPE_LABELS).map(([key, label]) => (<option key={key} value={key}>{label}</option>))}</select></div>
                  <div className="flex items-end gap-3">
                    <label className="flex items-center gap-2 text-sm text-slate-700"><input type="checkbox" checked={newField.required} onChange={(e) => setNewField((prev) => ({ ...prev, required: e.target.checked }))} className="rounded border-slate-300" />Obrigatório</label>
              ...
              {fieldsLoading ? (<div className="flex items-center justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-slate-400" /></div>) : fields.length === 0 ? (<div className="text-center py-8 text-sm text-slate-500">Nenhum campo personalizado cadastrado.</div>) : (
                <div className="space-y-3">
                  <h4 className="text-sm font-semibold text-slate-900">Campos Cadastrados ({fields.length})</h4>
                  {fields.map((field) => (
                    <div key={field.id} className="flex items-center gap-3 p-3 rounded-lg border border-slate-200 bg-white">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-medium text-slate-900">{field.name}</span>
                          <span className="text-xs text-slate-500 font-mono bg-slate-100 px-1.5 py-0.5 rounded">[[{field.placeholder}]]</span>
                          <span className="text-xs text-slate-500">{FIELD_TYPE_LABELS[field.field_type]}</span>
                      ...
                      <div className="flex items-center gap-1">
                        <button type="button" onClick={() => handleStartEditField(field)} className="p-2 text-slate-400 hover:text-amber-700 hover:bg-amber-50 rounded-lg" title="Editar campo">
                          <Pencil className="h-4 w-4" />
                        </button>
                        <button type="button" onClick={() => handleDeleteField(field.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg" title="Excluir campo">
            ...
            <div className="border-t border-slate-200 bg-slate-50 px-4 sm:px-6 py-3">
              <button type="button" onClick={handleCloseFieldsModal} className="w-full px-4 py-2.5 rounded-xl bg-slate-900 text-white text-sm font-semibold hover:bg-slate-800">Fechar</button>
            </div>
          </div>
        </div>,
        document.body
          ...
          <div className="pericia-light-modal__panel relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
            ...
            <div className="px-5 sm:px-6 py-5 border-b border-slate-200 flex items-start justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <h2 className="text-xl font-semibold text-slate-900 truncate max-w-md">{viewingPetition.name}</h2>
                </div>
              </div>
              <button type="button" onClick={handleCloseViewModal} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-xl"><X className="w-5 h-5" /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><p className="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-1">Status</p><span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ${viewingPetition.is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{viewingPetition.is_active ? 'Ativa' : 'Inativa'}</span></div>
              </div>
              {viewingPetition.description && <div><p className="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-1">Descrição</p><p className="text-sm text-slate-700">{viewingPetition.description}</p></div>}
              {viewingPetition.file_path && (<div className="flex items-center gap-3 p-3 rounded-lg bg-blue-50 border border-blue-200"><FileText className="h-5 w-5 text-blue-600" /><div className="flex-1"><p className="text-sm font-medium text-blue-900">{viewingPetition.file_name}</p><p className="text-xs text-blue-600">{viewingPetition.file_size ? `${(viewingPetition.file_size / 1024).toFixed(1)} KB` : 'Arquivo DOCX'}</p></div><button onClick={() => handleDownloadFile(viewingPetition)} className="px-3 py-1.5 rounded-lg bg-blue-600 text-white text-xs font-semibold hover:bg-blue-700 inline-flex items-center gap-1.5"><Download className="h-3.5 w-3.5" />Baixar</button></div>)}
              {!viewingPetition.file_path && viewingPetition.content && (<div><p className="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-1">Conteúdo</p><div className="p-3 rounded-lg bg-slate-50 border border-slate-200 max-h-64 overflow-y-auto"><pre className="text-xs text-slate-700 whitespace-pre-wrap font-mono">{viewingPetition.content}</pre></div></div>)}
              {viewLoading ? (<div className="flex items-center justify-center py-4"><Loader2 className="h-5 w-5 animate-spin text-slate-400" /></div>) : viewingFields.length > 0 && (<div><p className="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-2">Campos Personalizados ({viewingFields.length})</p><div className="space-y-2">{viewingFields.map((field) => (<div key={field.id} className="flex items-center gap-2 p-2 rounded-lg bg-slate-50 border border-slate-200"><span className="text-sm font-medium text-slate-900">{field.name}</span><span className="text-xs text-slate-500 font-mono bg-white px-1.5 py-0.5 rounded border">[[{field.placeholder}]]</span><span className="text-xs text-slate-500">{FIELD_TYPE_LABELS[field.field_type]}</span>{field.required && <span className="text-xs text-red-600">*</span>}</div>))}</div></div>)}
            </div>
            <div className="border-t border-slate-200 bg-slate-50 px-4 sm:px-6 py-3 flex gap-3">
              <button type="button" onClick={handleCloseViewModal} className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-semibold text-slate-700 hover:bg-slate-100">Fechar</button>
