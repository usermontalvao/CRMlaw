...
export const ProcessTimeline: React.FC<ProcessTimelineProps> = ({ 
  ...
  return (
    <div className="bg-[#f8f7f5] dark:bg-zinc-900 rounded-2xl shadow-2xl ring-1 ring-black/5 overflow-hidden w-[92vw] max-w-5xl max-h-[90vh] min-h-[520px] flex flex-col">
      {/* Header */}
      <div className="px-6 py-4 border-b border-[#e7e5df] dark:border-zinc-800 flex items-center justify-between bg-white dark:bg-zinc-900">
      ...
      <div className="px-6 py-5 bg-slate-50/70 dark:bg-zinc-800/50 border-b border-[#e7e5df] dark:border-zinc-700">
              ...
              <div key={stage.key} className="flex-1 flex flex-col items-center relative" title={stage.label}>
                ...
                <div className={`relative z-10 w-9 h-9 rounded-full flex items-center justify-center transition-all ${
                  isCurrent
                    ...
                    : 'bg-[#f8f7f5] dark:bg-zinc-800 text-slate-300 border border-[#e7e5df] dark:border-zinc-600'
                }`}>
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-[320px_1fr] overflow-hidden">
        ...
        <div className="border-b md:border-b-0 md:border-r border-[#e7e5df] dark:border-zinc-800 bg-[#f8f7f5] dark:bg-zinc-900 overflow-y-auto p-4 space-y-4">
          ...
          {/* Filtros compactos */}
          <div className="bg-slate-50/70 dark:bg-zinc-800/50 rounded-2xl border border-[#e7e5df] dark:border-zinc-700 p-4">
            ...
            <div className="flex flex-wrap gap-1.5 mb-3">
              ...
              {eventTypeCounts.despacho > 0 && (
                <button onClick={() => setFilterType(filterType === 'despacho' ? 'todos' : 'despacho')}
                  className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium transition border ${filterType === 'despacho' ? 'bg-slate-700 text-white border-slate-700' : 'bg-white/60 dark:bg-zinc-900/40 text-slate-700 dark:text-slate-200 border-[#e7e5df] dark:border-zinc-700 hover:bg-white dark:hover:bg-zinc-900/60'}`}>
            ...
            <div className="space-y-2">
              <div className="relative">
                ...
                <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Buscar..."
                  className="w-full pl-7 pr-2 py-2 bg-[#f8f7f5] dark:bg-zinc-900/50 border border-[#e7e5df] dark:border-zinc-700 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-[#f97316]/20 focus:border-[#f97316]/40" />
              </div>
              <select value={filterGrau} onChange={(e) => setFilterGrau(e.target.value)}
                className="w-full px-2 py-2 bg-[#f8f7f5]/70 dark:bg-zinc-900/50 border border-[#e7e5df] dark:border-zinc-700 rounded-xl text-xs">
          ...
          {processInfo && (processInfo.orgao || processInfo.poloAtivo.length > 0 || processInfo.poloPassivo.length > 0) && (
            <div className="bg-slate-50/70 dark:bg-zinc-800/50 rounded-2xl border border-[#e7e5df] dark:border-zinc-700 p-4 space-y-3">
          ...
          <button
            ...
            className={`w-full flex items-center justify-between px-2 py-1.5 rounded-lg border text-[10px] font-semibold transition ${
              groupByPhase
                ...
                : 'bg-[#f8f7f5] dark:bg-zinc-800 text-slate-600 border-[#e7e5df] dark:border-zinc-700 hover:bg-slate-50'
            }`}
          >
          ...
          <button
            ...
            className="w-full flex items-center justify-between px-2 py-1.5 bg-[#f8f7f5] dark:bg-zinc-800 rounded-lg border border-[#e7e5df] dark:border-zinc-700 hover:bg-slate-50 transition text-[10px] font-medium text-slate-600"
          >
            ...
            <div className="space-y-2 mt-2">
              ...
              {/* Resumo de Despachos */}
              {eventTypeCounts.despacho > 0 && (
                <div className="bg-slate-100 dark:bg-slate-800/50 border border-[#e7e5df] dark:border-slate-700 rounded-lg p-2.5">
                  <div className="flex items-center gap-2 mb-1.5">
                      ...
                      {phaseEvents.map((event) => {
                        ...
                        return (
                          <div key={event.id} className={`group rounded-xl border shadow-sm transition-all ${
                            ...
                            event.type === 'sentenca' ? 'bg-emerald-50/50 border-emerald-200' : 'bg-[#f8f7f5] border-[#e7e5df]'
            ...
            <div className="space-y-3">
              {filteredEvents.map((event, index) => {
                ...
                const getBgColor = () => {
                  ...
                  if (isLatest) return 'bg-[#f8f7f5] border-[#f97316]/30 ring-1 ring-[#f97316]/15 shadow-sm';
                  ...
                  return 'bg-[#f8f7f5] border-[#e7e5df]';
                };
      ...
      {/* Footer */}
      {events.length > 0 && (
        <div className="px-6 py-4 bg-white dark:bg-zinc-900 border-t border-[#e7e5df] dark:border-zinc-800">
