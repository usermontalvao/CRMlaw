...
import {
  ...
  Calendar, CalendarClock, Pencil, Clock, Check, Eye, PenLine, CheckCircle2, XCircle,
} from 'lucide-react';
...
/** Instante em que a assinatura se fechou (assinada ou recusada), ou null se ainda está em aberto. */
const closedAtOf = (req: import('../../types/signature.types').SignatureRequestWithSigners): string | null => {
  const signers = req.signers ?? [];
  const signed = signers.map(sg => sg.signed_at).filter(Boolean) as string[];
  const refused = signers.map(sg => sg.refused_at).filter(Boolean) as string[];
  const stamps = [...signed, ...refused];
  if (req.signed_at) stamps.push(req.signed_at);
  if (stamps.length > 0) return stamps.sort()[stamps.length - 1];
  // Status terminal sem carimbo de data — ainda assim é uma assinatura fechada.
  if (req.status === 'signed' || req.status === 'refused') return req.updated_at || req.created_at;
  return null;
};

export const ConversationSummaryBanner: React.FC<{ overview: ClientOverview | null; docStatus?: 'awaiting' | 'ready' | null; clientId?: string | null; embedded?: boolean; onOpenWorkspace?: WaOpenWorkspaceFn; onDismissDocReady?: () => void; onDismissTemplateFill?: (linkId: string) => void; onStopSignatureTracking?: (requestId: string) => void }> = ({ overview, docStatus, clientId, embedded, onOpenWorkspace, onDismissDocReady, onDismissTemplateFill, onStopSignatureTracking }) => {
  ...
  const fillByRequest = new Map(activeTrackedFills.filter(l => l.signature_request_id).map(l => [l.signature_request_id!, l]));
  // Assinatura JÁ CONCLUÍDA (assinada ou recusada) que ainda está sendo
  // acompanhada. Antes a faixa só existia enquanto a assinatura estava pendente:
  // no instante em que o cliente assinava — o evento que mais importa — o topo da
  // conversa ficava mudo. Mesma regra do chip da lista: só entra quando a
  // assinatura foi conduzida pela página pública (veio de um kit ou algum
  // signatário abriu o link) e é recente, para não acender assinatura presencial
  // nem ressuscitar caso antigo.
  const closedSignature = overview.signatures
    .filter(sig => !sig.archived_at && !sig.deleted_at && !(sig as any).wa_tracking_stopped)
    .filter(sig => Date.now() - new Date(sig.created_at).getTime() <= 30 * 86_400_000)
    .filter(sig => fillByRequest.has(sig.id) || (sig.signers ?? []).some(sg => !!sg.viewed_at || !!sg.opened_at || !!sg.last_seen_at))
    .map(sig => ({ sig, closedAt: closedAtOf(sig) }))
    .filter((x): x is { sig: typeof x.sig; closedAt: string } => !!x.closedAt)
    .sort((a, b) => b.closedAt.localeCompare(a.closedAt))[0]?.sig ?? null;
  const trackedFill = closedSignature
    ? fillByRequest.get(closedSignature.id) ?? null
    : activeTrackedFills
      .sort((a, b) => (b.last_seen_at || b.opened_at || b.submitted_at || b.created_at).localeCompare(a.last_seen_at || a.opened_at || a.submitted_at || a.created_at))[0] ?? null;
  // A assinatura do link só volta a valer se ELA ainda estiver sendo acompanhada:
  // ao dispensar a faixa, `wa_tracking_stopped` chega na assinatura antes de o
  // link recarregar, e sem este filtro o chip antigo reaparecia por um instante.
  const trackedReq = closedSignature
    ?? (trackedFill
      ? overview.signatures.find(sg => sg.id === trackedFill.signature_request_id
        && !sg.archived_at && !sg.deleted_at && !(sg as any).wa_tracking_stopped) ?? null
      : null);
  const trackedSigner = trackedReq?.signers?.find(sg => !!sg.signed_at || !!sg.refused_at)
    ?? trackedReq?.signers?.find(sg => sg.status !== 'signed' && !sg.refused_at)
    ?? trackedReq?.signers?.[0] ?? null;
  ...
  let FillIcon: React.ElementType = FilePlus;
  if (trackedFill || trackedReq) {
    if (trackedSigner?.signed_at || trackedReq?.status === 'signed' || trackedReq?.signed_at) {
      fillLabel = trackedFill ? 'Kit assinado' : 'Documento assinado';
      fillTone = 'bg-emerald-100 text-emerald-700';
      FillIcon = Check;
    } else if (trackedSigner?.refused_at || trackedReq?.status === 'refused') {
      fillLabel = trackedFill ? 'Kit recusado' : 'Assinatura recusada';
  ...
  const kitHistory: KitEvent[] = (trackedFill || trackedReq) ? ([
    trackedFill?.created_at && { at: trackedFill.created_at, label: 'Link do kit enviado', tone: 'text-slate-500' },
    (!trackedFill && trackedReq?.created_at) && { at: trackedReq.created_at, label: 'Documento enviado para assinatura', tone: 'text-slate-500' },
    ...
    (trackedSigner?.signed_at || trackedReq?.signed_at) && { at: (trackedSigner?.signed_at || trackedReq?.signed_at)!, label: trackedFill ? 'Assinou o kit' : 'Assinou o documento', tone: 'text-emerald-600' },
  ].filter(Boolean) as KitEvent[])
    .sort((a, b) => a.at.localeCompare(b.at)) : [];

  // Fechar o acompanhamento: pela assinatura quando ela existe (isso também
  // encerra o link do kit vinculado), senão pelo próprio link de preenchimento.
  const stopTracking = (trackedReq && onStopSignatureTracking) ? () => onStopSignatureTracking(trackedReq.id)
    : (trackedFill && onDismissTemplateFill) ? () => onDismissTemplateFill(trackedFill.id)
    : null;

  // Assinatura fechada ganha FAIXA PRÓPRIA, de ponta a ponta, abaixo do resumo —
  // um chip no meio dos outros passava batido justamente no aviso que o
  // escritório mais espera.
  const closedAt = closedSignature ? closedAtOf(closedSignature) : null;
  const closedRefused = !!closedSignature
    && (closedSignature.status === 'refused' || (closedSignature.signers ?? []).some(sg => !!sg.refused_at));
  const closedName = trackedFill?.template_name || closedSignature?.document_name || null;
  const closedSignerName = (closedSignature?.signers ?? []).find(sg => !!sg.signed_at || !!sg.refused_at)?.name || null;

  // A linha de resumo só existe se tiver o que dizer: sem isso, uma conversa que
  // só tem a assinatura assinada mostraria a tarja "Resumo" vazia acima da faixa.
  const hasSummaryRow = !!docStatus || (!closedSignature && !!fillLabel)
    || s.processCount > 0 || !!s.nextDeadline || (!!s.nextEvent && !!events[0])
    || s.pendingCount > 0 || openedSignatureCount > 0 || pendingSignatureCount > 0;

  // Sem contexto relevante → não polui o topo da thread. Fica DEPOIS do cálculo
  // da faixa de acompanhamento: uma assinatura concluída é motivo suficiente
  // para o banner existir, mesmo sem processo, prazo ou pendência.
  if (!hasSummaryRow && !closedSignature) return null;

  const closedBand = !closedSignature ? null : (
    <div className={`flex items-center flex-wrap gap-x-2 gap-y-1 border-b text-[12.5px] ${embedded ? 'px-3 py-2' : 'px-5 py-2.5'} ${closedRefused ? 'bg-rose-50 border-rose-200 text-rose-800' : 'bg-emerald-50 border-emerald-200 text-emerald-800'}`}>
      {closedRefused ? <XCircle size={16} className="flex-shrink-0" /> : <CheckCircle2 size={16} className="flex-shrink-0" />}
      <span className="font-bold">{closedRefused ? 'Cliente recusou a assinatura' : 'Cliente assinou'}</span>
      {closedName && <span className="truncate max-w-[280px] font-medium opacity-90">· {closedName}</span>}
      {closedAt && <span className="opacity-70">· {fmtDateTime(closedAt)}</span>}
      {closedSignerName && <span className="opacity-70">· {closedSignerName}</span>}
      {/* Ações juntas num bloco só: quando a faixa quebra em duas linhas (painel
          estreito), histórico e botão descem juntos em vez de se separarem. */}
      <span className="ml-auto inline-flex items-center gap-2">
      {kitHistory.length > 0 && (
        <HoverDetail width="w-80" trigger={
          <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[11px] font-semibold cursor-default ${closedRefused ? 'bg-rose-100 text-rose-700' : 'bg-emerald-100 text-emerald-700'}`}>
            <History size={12} /> Histórico
          </span>
        }>
          <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-2">
            {trackedFill
              ? `Histórico do kit${trackedFill.template_name ? ` · ${trackedFill.template_name}` : ''}`
              : `Histórico da assinatura${trackedReq?.document_name ? ` · ${trackedReq.document_name}` : ''}`}
          </p>
      ...
      {stopTracking && (
        <button onClick={stopTracking} title="Parar de acompanhar e interromper os lembretes automáticos"
          className={`inline-flex items-center gap-1 pl-1.5 pr-2 py-0.5 rounded-full border text-[11px] font-semibold transition hover:text-white hover:border-transparent ${closedRefused ? 'bg-white/70 border-rose-200 text-rose-700 hover:bg-rose-600' : 'bg-white/70 border-emerald-200 text-emerald-700 hover:bg-emerald-600'}`}>
          <X size={11} strokeWidth={2.75} /> Parar acompanhamento
        </button>
      )}
      </span>
    </div>
  ...
  // Fragmento, e não uma <div> de embrulho: as duas faixas continuam sendo
  // filhas DIRETAS da coluna da thread, cada uma ocupando a largura inteira
  // como a faixa única fazia antes.
  return (
    <>
      {hasSummaryRow && (
        <div className={`flex items-center gap-y-1 flex-wrap bg-amber-50/70 ${closedBand ? '' : 'border-b border-amber-100'} text-[12px] text-slate-600 ${embedded ? 'gap-x-3 px-3 py-1.5' : 'gap-x-4 px-5 py-2'}`}>
          ...
          {/* Assinatura fechada não vira chip aqui — ela tem a faixa própria abaixo. */}
          {!closedBand && fillLabel && (
            <HoverDetail width="w-80" trigger={
              <span className={`inline-flex items-center gap-1 pl-1.5 pr-1 py-0.5 rounded text-[11px] font-semibold ${fillTone}`}>
                <FillIcon size={12} /> {fillLabel}
                {stopTracking && (
                  <button onClick={stopTracking}
                    title="Parar de acompanhar e interromper os lembretes automáticos"
