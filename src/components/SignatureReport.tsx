...
import { CheckCircle, Download, FileText, User, X } from 'lucide-react';
...
const buildGoogleMapsUrl = (lat?: string | null, lng?: string | null) => {
...
const DetailSectionLabel: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div className="flex items-center gap-3 mb-4">
    <div className="w-[4px] h-[22px] flex-shrink-0" style={{ background: '#16a34a' }} />
    <span className="text-[13px] font-bold uppercase tracking-[0.03em]" style={{ color: '#0f172a' }}>{children}</span>
  </div>
);

const DataBlock: React.FC<{ label: string; value: React.ReactNode; mono?: boolean }> = ({ label, value, mono }) => (
  <div>
    <p className="mb-1 text-[10px] font-bold uppercase tracking-[0.08em]" style={{ color: '#94a3b8' }}>{label}</p>
    <div
      className={`min-h-[40px] rounded-xl px-4 py-2.5 text-[13px] leading-snug break-all ${mono ? 'font-mono' : 'font-medium'}`}
...
const SignatureReport: React.FC<SignatureReportProps> = ({ signer, request, creator, onClose }) => {
  ...
  }, [signer.signature_image_path, signer.facial_image_path, verificationUrl]);
  ...
  const locationUrl = buildGoogleMapsUrl(geo?.[0], geo?.[1]);
  const contact = (() => {
  ...
  const signerDataBlocks = [
    { label: 'Nome', value: signer.name },
    { label: 'Papel', value: signer.role || 'Signatário' },
    { label: 'Contato', value: contact || 'Não informado' },
    signer.cpf ? { label: 'CPF', value: signer.cpf, mono: true } : null,
    signer.signer_ip ? { label: 'Endereço IP', value: signer.signer_ip, mono: true } : null,
    geo?.[0] && geo?.[1]
      ? {
          label: 'Localização',
          value: locationUrl ? (
      ...
      : null,
    ua ? { label: 'Dispositivo', value: `${ua.browser} · ${ua.os} · ${ua.device}` } : null,
    {
      label: 'Autenticação',
      value:
    ...
    signer.terms_accepted_at
      ? {
          ...
          value: (
            <>
              Aceitos · versão {signer.terms_version || 'v1'}{' '}
              <a
      ...
      : null,
    signer.signed_at ? { label: 'Assinado em', value: fmtFull(signer.signed_at), mono: true } : null,
  ].filter(Boolean) as Array<{ label: string; value: React.ReactNode; mono?: boolean }>;

  const authFactorBlocks = authItems.map((item, index) => ({
    label:
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: '#f1f5f9' }}>
        <div className="flex flex-col items-center gap-3">
          <div className="w-8 h-8 rounded-full border-[3px] border-t-transparent animate-spin" style={{ borderColor: '#ea580c', borderTopColor: 'transparent' }} />
  ...
  return (
    <div className="min-h-screen print:bg-[#f8f7f5]" style={{ background: '#e8edf2' }}>
        ...
        <div className="max-w-6xl mx-auto px-5 h-[52px] flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {onClose && (
              <button
                onClick={onClose}
                className="w-[30px] h-[30px] rounded-lg flex items-center justify-center transition"
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 print:p-0 print:max-w-none">
        <div className="rounded-2xl overflow-hidden mb-4 print:rounded-none" style={{ background: '#0f172a', boxShadow: '0 2px 12px rgba(15,23,42,0.18)' }}>
          ...
          <div className="px-6 sm:px-8 py-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h1 className="text-[17px] font-bold leading-tight" style={{ color: 'white' }}>Certificado de Assinatura Eletrônica</h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full flex-shrink-0" style={{ background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.25)' }}>
                <div className="w-[6px] h-[6px] rounded-full" style={{ background: '#10b981' }} />
                <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: '#34d399' }}>Assinado · Válido</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="lg:col-span-2 flex flex-col gap-4">
            <div className="bg-[#f8f7f5] rounded-2xl print:rounded-none overflow-hidden" style={{ border: '1px solid #e8edf2', boxShadow: '0 1px 4px rgba(15,23,42,0.05)' }}>
              ...
              <div className="px-5 py-4 flex items-start gap-3">
                <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5" style={{ background: '#fff7ed', border: '1px solid #fed7aa' }}>
                  <FileText className="w-[17px] h-[17px]" style={{ color: '#ea580c' }} />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-[15px] break-words leading-snug" style={{ color: '#0f172a' }}>{request.document_name}</p>
            ...
            <div className="bg-[#f8f7f5] rounded-2xl print:rounded-none overflow-hidden" style={{ border: '1px solid #e8edf2', boxShadow: '0 1px 4px rgba(15,23,42,0.05)' }}>
              ...
              <div className="px-5 py-4 flex items-center gap-3" style={{ borderBottom: '1px solid #f8fafc' }}>
                {faceUrl ? (
                  ...
                  <div className="relative w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: '#f1f5f9', border: '1px solid #e2e8f0' }}>
                ...
                <div className="flex-shrink-0 text-right">
                  <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold" style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', color: '#15803d' }}>
              ...
              <div className="px-5 py-4" style={{ borderTop: '1px solid #f8fafc' }}>
                <DetailSectionLabel>Dados do signatário</DetailSectionLabel>
                <div className="grid grid-cols-1 gap-3">
                  {signerDataBlocks.map((row, i) => (
                    <DataBlock key={`${row.label}-${i}`} label={row.label} value={row.value} mono={row.mono} />
                  ))}
                </div>
              </div>

              {authFactorBlocks.length > 0 && (
                <div className="px-5 pb-4 pt-4" style={{ borderTop: '1px solid #f8fafc' }}>
                  <DetailSectionLabel>Fatores de autenticação</DetailSectionLabel>
                  <div className="grid grid-cols-1 gap-3">
                    {authFactorBlocks.map((item, i) => (
                      <DataBlock key={`${item.label}-${i}`} label={item.label} value={item.value} />
                    ))}
                  </div>
                </div>
              )}
              ...
              {sigUrl && (
                <div className="px-5 pb-4" style={{ borderTop: '1px solid #f8fafc' }}>
                  <p className="text-[10px] font-bold uppercase tracking-[0.14em] mt-3 mb-2" style={{ color: '#94a3b8' }}>Assinatura manuscrita</p>
                  <div className="inline-flex items-center justify-center px-6 py-3 rounded-xl" style={{ background: '#fafafa', border: '1.5px dashed #e2e8f0' }}>
          ...
          <div className="flex flex-col gap-4">
            {signer.verification_hash && (
              <div className="bg-[#f8f7f5] rounded-2xl print:rounded-none overflow-hidden" style={{ border: '1px solid #e8edf2', boxShadow: '0 1px 4px rgba(15,23,42,0.05)' }}>
                ...
                <div className="px-5 py-4">
                  <p className="font-mono text-[18px] font-bold tracking-[0.08em] break-all leading-relaxed" style={{ color: '#0f172a' }}>{signer.verification_hash}</p>
                  <p className="text-[11px] mt-2" style={{ color: '#94a3b8' }}>Use este código para verificar a autenticidade do documento a qualquer momento.</p>
                </div>
              </div>
            )}
            ...
            {signer.signed_pdf_sha256 && (
              <div className="bg-[#f8f7f5] rounded-2xl print:rounded-none overflow-hidden" style={{ border: '1px solid #e8edf2', boxShadow: '0 1px 4px rgba(15,23,42,0.05)' }}>
                ...
                <div className="px-5 py-4">
                  <p className="font-mono text-[10px] break-all leading-relaxed" style={{ color: '#475569' }}>{signer.signed_pdf_sha256}</p>
                  <p className="text-[10.5px] mt-2" style={{ color: '#94a3b8' }}>Garante que o documento não foi alterado após a assinatura.</p>
                </div>
              </div>
            )}
            ...
            <div className="rounded-2xl px-5 py-4" style={{ background: '#fafafa', border: '1px solid #f1f5f9' }}>
              <p className="text-[10.5px] leading-relaxed" style={{ color: '#94a3b8' }}>
                Assinatura eletrônica com validade jurídica conforme a <strong style={{ color: '#64748b' }}>MP 2.200-2/2001</strong> e a <strong style={{ color: '#64748b' }}>Lei 14.063/2020</strong>.
              </p>
              <p className="text-[10px] mt-2 font-mono" style={{ color: '#cbd5e1' }}>{request.id}</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 flex items-center justify-between px-1">
          ...
          <span className="text-[11px]" style={{ color: '#94a3b8' }}>Emitido em {fmtShort(new Date().toISOString())} · UTC-4 (Manaus)</span>
        </div>
      </div>
