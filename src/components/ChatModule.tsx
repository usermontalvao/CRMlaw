                  ...
                  {messages.map((msg, _idx) => {
                      ...
                      <div className={`flex flex-wrap gap-1 mt-1 ${isMine ? 'justify-end' : ''}`}>
                        {reactionChips.map(([emoji, info]) => (
                          <button
                            ...
                            className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-xs border transition ${info.mine ? 'bg-amber-100 border-amber-300 text-amber-800' : 'bg-white dark:bg-zinc-800 border-slate-200 dark:border-zinc-700 text-slate-600 dark:text-slate-300'}`}
                          >
                    ...
                    const Actions = !isDeleted ? (
                      <div className={`absolute top-0 ${isMine ? 'left-0 -translate-x-full pr-2' : 'right-0 translate-x-full pl-2'} opacity-0 group-hover:opacity-100 transition flex items-center gap-0.5`}>
                        <div className="flex items-center gap-0.5 bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-full px-1 py-0.5 shadow-sm">
                    ...
                    const ReactionPicker = reactionPickerFor === msg.id ? (
                      <div className={`absolute z-20 -top-9 ${isMine ? 'right-0' : 'left-0'} bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-full px-1.5 py-1 shadow-lg flex gap-0.5`}>
                        {QUICK.map((e) => (
