/**
 * O HISTÓRICO DE VERSÕES DO CRM — a fonte única.
 *
 * Cada commit que sobe para produção vira uma entrada aqui, com o mesmo número
 * que está no `package.json`. Esta lista não é enfeite de documentação: é o que
 * o aviso "o que mudou" mostra a quem recarrega a página depois de um deploy
 * (`components/VersionNewsPanel.tsx`). Commit sem entrada aqui é trabalho que
 * ninguém no escritório fica sabendo que existe.
 *
 * A ordem importa: da versão MAIS NOVA para a mais antiga. O aviso confia nisso
 * para empilhar as versões que a pessoa ainda não viu.
 *
 * Estes dados moravam dentro de `components/DocsChangesPage.tsx`, um componente
 * de 24 mil linhas que não é montado em lugar nenhum. Ficaram aqui para poder
 * ser lidos sem arrastar a página inteira junto.
 */
        ...
        changes: [
          {
            ...
            title: 'O CRM conta o que mudou depois de cada atualizacao',
            description: 'Uma versao nova entrava no ar e ninguem no escritorio ficava sabendo: o Changelog existia, mas so via quem fosse atras. Agora, quando a pagina e recarregada e o pacote que chega e de uma versao que aquela pessoa ainda nao viu, um painel entra pela direita com o que veio de novo e o que foi corrigido, separado por tipo e marcado por modulo. Aparece UMA VEZ por versao — fechar, ignorar ou trocar de tela da no mesmo — e apenas em jurius.com.br: em localhost o pacote muda a cada salvamento e o aviso viraria interrupcao.',
          },
          {
            type: 'feature' as const,
            title: 'Varios commits no mesmo envio viram um aviso so',
            description: 'Um envio para producao costuma levar varios commits, e cada commit e uma versao propria. O painel nao anuncia so a mais recente: ele empilha TODAS as versoes entre a ultima vista e a atual, da mais nova para a mais antiga. Quem passou dois dias fora recebe os dois dias, com teto de dez versoes — o que passar disso fica no Changelog. Quem abre o CRM pela primeira vez nao recebe nada: o sistema apenas guarda a versao atual e fica quieto.',
          },
          {
            type: 'fix' as const,
            title: 'O Changelog do sistema voltou a ser o de verdade',
            description: 'A pagina de documentacao mostrava uma lista digitada a mao que parou na versao 1.3.29, de dezembro de 2025, e anunciava "versao atual 1.10.016" enquanto o CRM ja ia na 1.10.338. Agora ela le o historico real, o mesmo que alimenta o aviso, e a versao exibida vem do proprio pacote em execucao. A busca passou a procurar tambem no titulo de cada mudanca, que e onde estao as palavras de que alguem se lembraria.',
          },
        ],
      },
    ],
