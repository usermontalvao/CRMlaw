    ...
    <div className="flex flex-col gap-5">
        ...
        {(['all', 'unread'] as Tab[]).map((t) => {
          ...
          return (
            <button key={t} onClick={() => setTab(t)}
              ...
              <span className={`tabular-nums text-[11px] font-semibold ${on ? 'text-orange-700' : 'text-slate-400'}`}>{t === 'all' ? items.length : unreadCount}</span>
            </button>
          );
        })}
      </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description={tab === 'unread' ? 'Você já leu todas as suas notificações.' : 'Quando houver novidades nos seus processos, você verá aqui.'}
        />
      ) : (
                ...
                {list.map((n) => {
                  ...
                  return (
                    <button
                      ...
                      className={`group flex w-full items-start gap-3 rounded-xl border bg-white p-4 text-left transition ${
                        isClickable ? 'cursor-pointer hover:border-slate-300' : 'cursor-default'
                      } ${unread ? 'border-l-[3px] border-l-orange-500 border-slate-200' : 'border-slate-200'}`}
                    >
