...
<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="600" class="container" style="width:100%;max-width:600px;background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 20px 50px -20px rgba(20,28,52,0.22),0 4px 14px -6px rgba(20,28,52,0.10);">
    ...
    <table role="presentation" cellpadding="0" cellspacing="0" border="0">
      <tr>
        ...
        <td valign="middle">
          ...
          <div style="margin-top:6px;font-family:'Space Grotesk',Arial,Helvetica,sans-serif;font-size:8px;font-weight:600;letter-spacing:0.28em;color:#A0958C;text-transform:uppercase;">GESTAO JURIDICA INTELIGENTE</div>
        </td>
      </tr>
    </table>
  </td></tr>
  ...
  <tr><td style="background:#F0FDF4;border-bottom:2px solid #BBF7D0;padding:12px 36px;">
    <table role="presentation" cellpadding="0" cellspacing="0" border="0"><tr>
      ...
      <td valign="middle" style="padding-left:10px;"><span style="font-family:Arial,Helvetica,sans-serif;font-size:13px;font-weight:700;color:#15803d;">${statusText} &mdash; ${fmtDate(p.signedAt)}</span></td>
    </tr></table>
  </td></tr>
  ...
  <tr><td class="card-pad" style="padding:32px 36px 12px;">
    ...
    <div style="font-family:Georgia,'Times New Roman',serif;font-size:28px;line-height:1.2;color:#16213A;letter-spacing:-0.015em;">${heading}</div>
    <p style="margin:14px 0 0;font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:1.65;color:#54607A;">${intro}</p>
  </td></tr>
  ...
  <tr><td style="padding:20px 36px 4px;">
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#FFF6EE;border:1px solid #FAD9C0;border-radius:16px;overflow:hidden;">
      ...
      <tr><td style="padding:22px 24px 24px;">
        <div style="font-family:Georgia,'Times New Roman',serif;font-size:20px;color:#16213A;font-weight:bold;margin-bottom:20px;">${docTitle}</div>
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" class="two-col">
          <tr>
            <td width="50%" valign="top" style="padding-right:12px;padding-bottom:16px;">
              <div style="font-family:Arial,Helvetica,sans-serif;font-size:10px;font-weight:700;letter-spacing:0.12em;color:#B0834F;text-transform:uppercase;margin-bottom:7px;">Signatario</div>
  ...
  <tr><td style="padding:12px 36px 4px;">
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#F8F9FB;border:1px solid #E8EAF0;border-radius:12px;">
      <tr><td style="padding:12px 16px;"><div style="font-family:Arial,Helvetica,sans-serif;font-size:11px;color:#54607A;line-height:1.6;">&#9989; Validade juridica conforme a <strong>MP 2.200-2/2001</strong> e a <strong>Lei 14.063/2020</strong>.</div></td></tr>
    </table>
  </td></tr>
  ...
  <tr><td style="padding:18px 24px 16px;border-top:1px solid #F0F1F4;text-align:center;background:#F8F9FB;">
    <div style="font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#5C667C;font-weight:600;">Jurius &bull; Gestao Juridica Inteligente</div>
    <div style="margin-top:6px;font-family:Arial,Helvetica,sans-serif;font-size:10px;line-height:1.6;color:#9AA2B2;">Este e-mail foi enviado automaticamente. Nao responda esta mensagem.<br/>&copy; 2026 Jurius. Todos os direitos reservados.</div>
  </td></tr>
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const isPerDocument = req_.signature_model === 'per_document'
    let requestDocuments: any[] = []
    if (isPerDocument) {
      const { data: docs, error: docsErr } = await supabase
        .from('signature_request_documents')
        .select('id, display_name, verification_code, signed_file_path, document_type, document_key, sort_order, status')
        .eq('signature_request_id', requestId)
        .order('sort_order', { ascending: true })
        .order('created_at', { ascending: true })
      if (docsErr) {
        console.error('Erro ao carregar documentos do envelope:', docsErr)
      } else {
    ...
    for (const signer of signedOnes) {
      ...
      if (!email || isPlaceholder || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        failed.push({ email: email ?? '(sem email)', error: 'Email invalido' })
        continue
      }
      ...
      const html = buildHtml({
        ...
        signerName: signer.name ?? 'Signatario',
