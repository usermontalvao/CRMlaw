-- O endereço da perícia também no portal do cliente.
--
-- Mesma função de antes, com dois campos a mais: sem eles o cliente vê a data
-- da perícia no portal e continua sem saber onde comparecer — exatamente o que
-- o campo novo foi criado para resolver.
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'pericia_social_local', r.pericia_social_local,
    'pericia_medica_at',    r.pericia_medica_at,
    'pericia_medica_local', r.pericia_medica_local,
  ...
  IF to_regclass('public.requirement_status_history') IS NOT NULL THEN
    SELECT COALESCE(
      jsonb_agg(
        jsonb_build_object(
          ...
          'changed_by_name', pr.name
        )
        ORDER BY h.changed_at DESC
      ),
      '[]'::jsonb
    ) INTO v_history
    FROM public.requirement_status_history h
    LEFT JOIN public.profiles pr ON pr.user_id = h.changed_by
    WHERE h.requirement_id = p_requirement_id;
  END IF;

  RETURN v_req
    || jsonb_build_object('appointments',     v_appointments)
    || jsonb_build_object('linked_processes', v_linked_procs)
    || jsonb_build_object('status_history',   v_history);
END;
$function$;
