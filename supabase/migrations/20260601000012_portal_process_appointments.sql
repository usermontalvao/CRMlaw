-- Atualiza portal_get_process para incluir compromissos (calendar_events)
-- vinculados ao processo via process_id.
-- Exibe data futura e passada recente (últimos 30 dias + futuros).
...
BEGIN
  ...
  -- Compromissos da agenda vinculados a este processo (process_id)
  -- Retorna futuros + passados recentes (últimos 30 dias), não-privados.
...
-- Garante execute para portal_client (caso ainda não tenha sido concedido
-- por execução posterior à migration de roles).
DO $$
BEGIN
